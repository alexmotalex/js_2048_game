...
button.addEventListener("click", ()=>{
    if (button.className === "button restart") game.restart();
    game.start();
    updateUi();
...
function updateUi() {
    ...
    if (currentStatus === "win") winMessage.classList.remove("hidden");
    else if (currentStatus === "lose") loseMessage.classList.remove("hidden");
}
...
class Game {
    ...
    findEmptyTiles() {
        ...
        for(let row = 0; row < BOARD_SIZE; row++){
            for(let col = 0; col < BOARD_SIZE; col++)if (this.state[row][col] === 0) emptyTiles.push([
                row,
                col
            ]);
        }
    ...
    // --- Game Status Updates ---
    updateGameStatus() {
        if (this.hasWinningTile()) this.status = "win";
    ...
    isGameOver() {
        ...
        const canMerge = this.state.some((row, rowIndex)=>{
            return row.some((value, colIndex)=>{
                ...
                if (colIndex < BOARD_SIZE - 1 && value === row[colIndex + 1]) return true;
                // Check down neighbor
                if (rowIndex < BOARD_SIZE - 1 && value === this.state[rowIndex + 1][colIndex]) return true;
                return false;
            });
        });
    ...
    canMergeWithNeighbor(row, col, tile) {
        ...
        const rightNeighbor = col < BOARD_SIZE - 1 && tile === this.state[row][col + 1];
        const downNeighbor = row < BOARD_SIZE - 1 && tile === this.state[row + 1][col];
